<!-- MAIN NAVIGATION -->
<nav id="t3-mainnav" class="wrap t3-mainnav ">
  <div class="container navbar">
    <div class="navbar-inner">

      <button type="button" class="btn btn-navbar" data-toggle="collapse" data-target=".nav-collapse">
        <span class="icon-bar"></span>
        <span class="icon-bar"></span>
        <span class="icon-bar"></span>
      </button>

<div class="nav-collapse collapse always-show">
<div class="t3-megamenu animate slide" data-duration="400">

<ul class="nav level0">
	<li class="current active" data-id="435" data-level="1" data-xicon="icon-home">
	<a  href="index.php"  ><i class="icon-home"></i>หน้าแรก </a>
	</li>

	<li class="dropdown mega" data-id="233" data-level="1" data-xicon="icon-pencil">
		<a class=" dropdown-toggle" href="#"  data-toggle="dropdown">
		  <i class="icon-pencil"></i>เกี่ยวกับคณะ <b class="caret"></b></a>
		<div class="nav-child dropdown-menu mega-dropdown-menu" style="width:250px" >
		<div class="mega-dropdown-inner">
		<div class="row-fluid">
		<div class="span12 mega-col-nav" data-width="12">
		<div class="mega-inner">
		   <ul class="mega-nav level1">
		        <li data-level="2">
				<a href="about.php?page=vision">วิสัยทัศน์/พันธกิจ</a>
				</li>
				<li data-level="2">
				<a href="about.php?page=history">ประวัติคณะ</a>
				</li>
				<li data-level="2">
				<a  href="about.php?page=organizational">โครงสร้างองค์กร</a>
				</li>
				<li class="mega mega-group" data-level="2" data-group="1">
				<a class=" mega-group-title" href="about.php?page=symblo"  >อัตลักษณ์ของคณะ</a>
				<div class="nav-child mega-group-ct"  ><div class="mega-dropdown-inner">
				<div class="row-fluid">
				<div class="span12 mega-col-nav" data-width="12"><div class="mega-inner">
					<ul class="mega-nav level2">
						<li data-level="3">
						<a  href="about.php?page=symblo">ตราสัญลักษณ์</a>
						</li>
						<li data-level="3">
						<a  href="about.php?page=flower"  >ดอกไม้ประจำคณะ</a>
						</li>
						<li data-level="3">
						<a  href="about.php?page=color"  >สีประจำคณะ</a>
						</li>
						<li data-level="3">
						<a  href="about.php?page=music"  >เพลงประจำคณะ</a>
						</li>
			        </ul>
				</div>
				</div>
				</div>
				</div>
				</div>
	            </li>
			<li data-level="2">
			<a href="#">ผลงาน/รางวัล</a>
			</li>
		       <li data-level="2">
			<a href="about.php?page=contact">สถานที่ตั้งและการติดต่อ</a>
			</li>
          </ul>
</div>
</div>


	</div>
	</div></div>
</li>

<li class="dropdown mega" data-id="238" data-level="1" data-xicon="icon-cog">
<a class=" dropdown-toggle" href="#"   data-toggle="dropdown"><i class="icon-cog"></i>หลักสูตร <b class="caret"></b></a>
<div class="nav-child dropdown-menu mega-dropdown-menu"  ><div class="mega-dropdown-inner">
<div class="row-fluid">
<div class="span12 mega-col-nav" data-width="12"><div class="mega-inner">
<ul class="mega-nav level1">
<li  data-id="492" data-level="2">
<a href="program.php?page=bachelor"  >หลักสูตรปริญญาตรี</a>
</li>
<li  data-id="493" data-level="2">
<a href="program.php?page=master"  >หลักสูตรปริญญาโท</a>
</li>
</ul>
</div></div>
</div>
</div></div>
</li>

<li class="dropdown mega mega-align-justify sub-hidden-collapse" data-id="489" data-level="1" data-alignsub="justify" data-xicon="icon-briefcase" data-hidewcol="1">
<a class=" dropdown-toggle" href="#"   data-toggle="dropdown"><i class="icon-sitemap"></i>สาขาวิชา/หน่วยงาน <b class="caret"></b></a>
<div class="nav-child dropdown-menu mega-dropdown-menu span12"  >
<div class="mega-dropdown-inner">
<div class="row-fluid">

<div class="span4 mega-col-module" data-width="4" data-position="207">
<div class="mega-inner">
<div class="t3-module module  portfolio " id="Mod207">

<div class="module-inner">
<h3 class="module-title">
<span><a href="branch.php?page=hospital" style="color:#000" onMouseOver="this.style.color='#d7ae1c'" onMouseOut="this.style.color='#000'">โรงพยาบาลการแพทย์แผนไทย</a></span></h3>
</div>
<div class="module-inner">
<h3 class="module-title "><span>สาขาวิชา</span></h3>
     <div class="module-ct">	
     <div class="row-fluid">
     <div class="span12 mega-col-nav" data-width="12"><div class="mega-inner">
	<ul class="mega-nav level2">
		<li data-level="3">
		<a  href="branch.php?page=medicine">สาขาวิชาเวชกรรมไทย</a>
		</li>
		<li data-level="3">
		<a  href="branch.php?page=drugs">สาขาวิชาเภสัชกรรมไทย</a>
		</li>
		<li data-level="3">
		<a  href="branch.php?page=massage">สาขาวิชานวดไทย</a>
		</li>
		<li data-level="3">
		<a  href="branch.php?page=midwifery">สาขาวิชาผดุงครรภ์ไทย</a>
		</li>
	 </ul>
      </div>
      </div>
      </div>
</div>


</div></div>
</div></div>

<div class="span4 mega-col-module" data-width="4" data-position="188"><div class="mega-inner">
<div class="t3-module module  " id="Mod188"><div class="module-inner"><h3 class="module-title ">
<span>ฝ่ายสนับสนุนวิชาการ</span></h3><div class="module-ct">
     <div class="row-fluid">
				<div class="span12 mega-col-nav" data-width="12"><div class="mega-inner">
					<ul class="mega-nav level2">
						<li  data-id="560" data-level="3">
						<a href="units/research/" target="_blank">งานวิจัยและบัณฑิตศึกษา</a>
						</li>
						<li  data-id="561" data-level="3">
						<a  href="units/clerkship/" target="_blank" >งานฝึกปฏิบัติงานเชิงวิชาชีพ</a>
						</li>
						<li  data-id="559" data-level="3">
						<a  href="units/comprehensive/" target="_blank">งานการสอบประมวลความรอบรู้</a>
						</li>
						<li  data-id="559" data-level="3">
						<a  href="units/certificate/" target="_blank">งานการสอบใบประกอบโรคศิลปะ</a>
						</li>
						<li  data-id="559" data-level="3">
						<a href="units/cai/" target="_blank">งานสื่อการเรียนรู้</a>
						</li>
						<li  data-id="559" data-level="3">
						<a  href="units/it/" target="_blank">งานเทคโนโลยีสารสนเทศ</a>
						</li>
			        </ul>
				</div>
				</div>
		</div>
</div></div>
</div>
</div>
</div>

<div class="span4 mega-col-module" data-width="4" data-position="188"><div class="mega-inner">
<div class="t3-module module  " id="Mod188"><div class="module-inner"><h3 class="module-title ">
<span>ฝ่ายบริหารงานทั่วไป</span></h3><div class="module-ct">
     <div class="row-fluid">
				<div class="span12 mega-col-nav" data-width="12"><div class="mega-inner">
					<ul class="mega-nav level2">
						<li  data-level="3">
						<a  href="units/personel/" target="_blank" >งานการเจ้าหน้าที่</a>
						</li>
						<li  data-level="3">
						<a  href="http://goo.gl/9qyLDy" target="_blank"  >งานการเงิน</a>
						</li>
						<li  data-level="3">
						<a href="http://goo.gl/8N7K4S" target="_blank"  >งานการประกันคุณภาพ</a>
						</li>
						<li  data-level="3">
						<a  href="http://goo.gl/htsABg" target="_blank" >งานนโยบายและแผน</a>
						</li>
                         <li data-level="3">
						<a  href="http://goo.gl/NRGmCk" target="_blank" >งานพัสดุ (ข้อมูลบัญชีวัสดุ)</a>
						</li>
			        </ul>
				</div>
				</div>
		</div>
</div></div>
</div>
</div>
</div>

</div>
</div></div>
</li>

<li class="dropdown mega" data-level="1" data-xicon="icon-text-width">
		<a class=" dropdown-toggle" href="#"  data-toggle="dropdown">
		  <i class="icon-user"></i>บุคลากร <b class="caret"></b></a>
		<div class="nav-child dropdown-menu mega-dropdown-menu" style="width:280px" >
		<div class="mega-dropdown-inner">
		<div class="row-fluid">
		<div class="span12 mega-col-nav" data-width="12">
		<div class="mega-inner">
		   <ul class="mega-nav level1">
		        <li data-level="2">
				<a href="staff.php?page=manager">ผู้บริหาร</a>
				</li>
				<li class="mega mega-group" data-level="2" data-group="1">
				<a class=" mega-group-title" href="#"  >คณาจารย์</a>
				<div class="nav-child mega-group-ct"  ><div class="mega-dropdown-inner">
				<div class="row-fluid">
				<div class="span12 mega-col-nav" data-width="12"><div class="mega-inner">
					<ul class="mega-nav level2">
						<li  data-level="3">
						<a  href="staff.php?page=medicine">สาขาวิชาเวชกรรมไทย</a>
						</li>
						<li data-level="3">
						<a  href="staff.php?page=drugs">สาขาวิชาเภสัชกรรมไทย</a>
						</li>
						<li data-level="3">
						<a  href="staff.php?page=mm">สาขาวิชานวดไทย/ผดุงครรภ์ไทย</a>
						</li>
			        </ul>
				</div>
				</div>
				</div>
				</div>
				</div>
	            </li>
				<li data-level="2">
				<a href="staff.php?page=general">ฝ่ายบริหารงานทั่วไป</a>
				</li>
				<li data-level="2">
				<a href="staff.php?page=academic">ฝ่ายสนับสนุนวิชาการ</a>
				</li>
				<li data-level="2">
				<a href="staff.php?page=subject">สายสนับสาขาวิชา</a>
				</li>
				<li data-level="2">
				<a href="staff.php?page=hospital">โรงพยาบาลการแพทย์แผนไทย</a>
				</li>
          </ul>
</div>
</div>


	</div>
	</div></div>
</li>


<li data-level="1" data-xicon="icon-globe">
<a  href="/english/index.php"  ><i class="icon-globe"></i>English</a>
</li>
</ul>
</div>
        		</div>
    </div>
  </div>
</nav>
<!-- //MAIN NAVIGATION -->  
